...
export class AttachmentsService {
  ...
  async generateSecureDownload(
    ...
    this.validateOrganizationScope(
    ...
    const expiresInSeconds =
      ...
      this.storageService.getDefaultPresignedExpirySeconds();
  ...
  private async findInternalAttachmentById(
    id: string,
  ): Promise<AttachmentInternal | null> {
    return this.prisma.attachment.findUnique({
      where: { id },
  ...
  private validateOrganizationScope(
    ...
    if (!requesterOrganizationId) {
      if (requireRequesterScope) {
        throw new ForbiddenException({
          ...
          message:
            'Se requiere contexto de organización para acceder al adjunto.',
          error: 'Forbidden',
        });
      }
      return;
    ...
    if (requesterOrganizationId !== resourceOrganizationId) {
      throw new ForbiddenException({
        ...
        code: ErrorCode.FILE_SCOPE_MISMATCH,
        message:
          'La organización del solicitante no coincide con la del adjunto.',
        error: 'Forbidden',
      });
    }
  }
