/**
 * ReceivablesReportPage — Reporte de cuentas por cobrar con aging de cartera.
 *
 * KPIs: total pendiente, vencidas, por vencer en 30 días, % cartera vencida.
 * Aging: tabla por contraparte (corriente, 1-30d, 31-60d, 61-90d, +90d).
 * Detalle: tabla filtrable + exportación CSV.
 *
 * Task origen: T-1605 (Fase 16 Bloque 2)
...
// ─── Computar aging por contraparte ──────────────────────────────────────────
...
function computeAging(items) {
  ...
  for (const r of items) {
    if (!ACTIVE_STATUSES.has(r.status)) continue;
    const cpName = r.counterpartyName ?? r.counterparty ?? r.description ?? "Sin contraparte";
    if (!map[cpName]) {
...
function KpiCards({ kpis }) {
  return (
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      {[
        { label: "Total por cobrar", value: formatMoney(kpis.totalPending), color: "text-ink-900" },
        { label: "Cartera vencida", value: formatMoney(kpis.totalOverdue), color: "text-error" },
        { label: "Vence en 30 días", value: formatMoney(kpis.dueSoon), color: "text-amber-600" },
        { label: "% cartera vencida", value: `${kpis.pctOverdue.toFixed(1)}%`, color: kpis.pctOverdue > 20 ? "text-error" : "text-ink-700" },
  ...
  return (
    <div className="rounded-xl border border-ink-200 overflow-hidden">
      <div className="border-b border-ink-100 bg-surface-subtle px-5 py-3">
        <h3 className="text-xs font-semibold uppercase tracking-wide text-ink-500">Aging por contraparte</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full text-xs">
          <thead>
            <tr className="border-b border-ink-50">
              <th className="px-4 py-2.5 pl-5 text-left font-semibold uppercase tracking-wide text-ink-400">Contraparte</th>
              {AGING_BUCKETS.map((b) => (
...
export default function ReceivablesReportPage() {
  ...
  // Filtrado para la tabla de detalle
  const filteredItems = useMemo(() => {
    if (!activeFilters) return allReceivables;
  ...
  function handleExportCsv() {
    ...
    exportToCsv(
      filteredItems,
      receivablesPayablesColumns(STATUS_LABELS, pending),
      `cuentas-por-cobrar-${date}`,
    );
    logExport({ reportName: "ReceivablesReport", format: "CSV", filters: activeFilters ?? {}, rowCount: filteredItems.length });
  }
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Reporte CxC"
        description="Cuentas por cobrar — cartera, aging y detalle"
      />
            ...
            <div className="flex items-center justify-between px-5 py-2.5 border-b border-ink-50">
              ...
              <div className="flex gap-2">
                {[
                  ...
                  { label: "XLSX", onClick: () => { const d = new Date().toISOString().slice(0,10); exportToXlsx([cxcCxpDetailSheet(filteredItems, STATUS_LABELS, pending, "CxC")], `cuentas-por-cobrar-${d}`); logExport({ reportName: "ReceivablesReport", format: "XLSX", filters: activeFilters ?? {}, rowCount: filteredItems.length }); } },
                  { label: "PDF", onClick: () => { const d = new Date().toISOString().slice(0,10); exportToPdf({ title: "Reporte CxC", sections: [{ title: "Detalle", columns: cxcCxpPdfColumns(STATUS_LABELS, pending), rows: filteredItems }] }, `cuentas-por-cobrar-${d}`); logExport({ reportName: "ReceivablesReport", format: "PDF", filters: activeFilters ?? {}, rowCount: filteredItems.length }); } },
                ].map(({ label, onClick }) => (
                  <button
                    key={label}
                    type="button"
