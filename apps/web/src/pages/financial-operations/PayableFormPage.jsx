...
/**
 * PayableFormPage — Formulario para crear/editar una CxP.
 */
...
export default function PayableFormPage() {
  ...
  const { data: existing, isLoading: loadingExisting } = usePayable(isEdit ? id : null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setInitialForm(form);
      navigate('/financial-operations/payables');
    } catch (err) {
      handleError(err);
    }
  };
  ...
  if (isEdit && loadingExisting) {
    return (
      <div className="space-y-6">
        ...
        <FinOpsLoadingState message="Cargando datos de la CxP..." />
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-text-secondary mt-1">
          Registra un pago pendiente a un proveedor o tercero
        </p>
      </div>
        ...
        <Card>
          <div className="p-4 md:p-6 grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5">
            ...
            <Select
              ...
              helpText="Cuenta desde donde se realizará el pago"
            />
            <Select
              label="Estado"
            ...
            {isEdit && (
              <Input
                ...
                className="font-mono"
                helpText="Monto acumulado de pagos realizados"
              />
            )}
            <div className={isEdit ? '' : 'md:col-span-2'}>
              <Textarea
                label="Descripción"
                placeholder="Describe esta cuenta por pagar..."
          ...
          <div className="px-4 py-3 md:px-6 md:py-4 border-t border-border flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-3">
            <Button
              type="button"
              variant="secondary"
              onClick={() => navigate('/financial-operations/payables')}
            >
              Cancelar
            </Button>
            <Button type="submit" variant="primary" disabled={isSaving}>
              {isSaving ? 'Guardando...' : isEdit ? 'Guardar cambios' : 'Registrar CxP'}
            </Button>
          </div>
        </Card>
      </form>
