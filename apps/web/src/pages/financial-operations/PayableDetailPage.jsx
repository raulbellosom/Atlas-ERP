...
export default function PayableDetailPage() {
  ...
  const { data: payable, isLoading, error } = usePayable(id);
  const { data: bankAccounts = [] } = useBankAccounts(organizationId);
  const registerPaymentMutation = useRegisterPayablePayment();
  ...
  if (error || !payable) return <EmptyState title="Error" description="No se pudo cargar la CxP" />;

  const isPaid = payable.status === 'PAID' || payable.status === 'CANCELED';
  const pendingAmount = Number(payable.amount) - Number(payable.amountPaid);
  ...
  const breadcrumbs = [
    { label: 'Tesorería' },
    { label: 'Cuentas por pagar', to: '/financial-operations/payables' },
    { label: 'Detalle' },
  ];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-text-primary">
            CxP: {payable.reference || payable.id}
          </h1>
          <p className="text-sm text-text-secondary mt-1">
            Monto total: {Number(payable.amount).toLocaleString()} {payable.currencyCode}
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            variant="secondary"
            onClick={() => navigate(`/financial-operations/payables/${id}/edit`)}
          >
            Editar
          </Button>
          {!isPaid && (
        ...
        <Card className="md:col-span-2 p-6">
          ...
          <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
            <div>
              ...
              <dd className="mt-1 text-sm text-text-primary">{payable.status}</dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-text-secondary">Monto pagado</dt>
              <dd className="mt-1 text-sm text-text-primary">
                {Number(payable.amountPaid).toLocaleString()} {payable.currencyCode}
              </dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-text-secondary">Fecha de emisión</dt>
              <dd className="mt-1 text-sm text-text-primary">
                {new Date(payable.issuedAt).toLocaleDateString()}
              </dd>
            </div>
            <div>
              <dt className="text-sm font-medium text-text-secondary">Fecha de vencimiento</dt>
              <dd className="mt-1 text-sm text-text-primary">
                {payable.dueAt ? new Date(payable.dueAt).toLocaleDateString() : 'N/A'}
              </dd>
            </div>
      ...
      >
        <form onSubmit={handlePaymentSubmit} className="p-4 space-y-4">
          <Input
            ...
            required
            max={pendingAmount}
            helpText={`Pendiente: ${pendingAmount.toLocaleString()} ${payable.currencyCode}`}
          />
          <Select
