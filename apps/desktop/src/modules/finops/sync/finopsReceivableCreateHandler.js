/**
 * Handler de sync para receivable.create.
 *
 * Al desencolar: POST /api/v1/receivables-lite con idempotency key.
 * Actualiza finops_cxc_cache: id local → id backend, status → PENDING.
 *
...
export async function handleFinopsReceivableCreate(item) {
  ...
  if (!backendId) {
    throw new Error(`[finopsReceivableCreateHandler] Backend no retornó id. Payload: ${JSON.stringify(created)}`);
  }

  await sqliteExecute(
    `UPDATE finops_cxc_cache
