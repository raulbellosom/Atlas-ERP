/**
 * CreateReceivableFormDesktop — formulario de cuenta por cobrar para desktop.
 *
 ...
 */
export default function CreateReceivableFormDesktop({ onSubmit, isOffline }) {
  const { values, hasDraft, draftLoaded, updateField, discardDraft, clearDraftOnSuccess } =
    useFormDraft("receivable", INITIAL);
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      {isOffline && (
        <p className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-2 text-sm text-amber-700">
          ⚠️ Modo offline — la cuenta por cobrar se encolará para sincronizar al reconectar.
        </p>
      )}

      <div>
        <label className="block text-sm font-medium text-slate-700">Contraparte / Cliente</label>
        <input
          type="text"
          required
      ...
      >
        {isOffline ? "Guardar (offline)" : "Guardar cuenta por cobrar"}
      </button>
    </form>
  );
}
